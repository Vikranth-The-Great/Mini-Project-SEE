...
export default function DeliveryLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post('/api/delivery/auth/login', form);
    ...
    <div className="auth-wrapper">
      <div className="auth-card">
        ...
        <p style={{ textAlign: 'center', marginTop: '1rem', fontSize: '.9rem', color: 'var(--muted)' }}>
          New delivery partner? <Link to="/delivery/signup" style={{ color: 'var(--green)', fontWeight: 600 }}>Sign Up</Link>
        </p>
      </div>
    </div>
  );
